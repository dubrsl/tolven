...
public class CertificateHelper {
    ...
    public void updateCredentials(TolvenPerson tolvenPerson, String encodedUserPKCS12, char[] userPassword) {
        KeyStore keyStore = null;
        try {
            try {
                byte[] userPKCS12 = Base64.decodeBase64(encodedUserPKCS12.getBytes("UTF-8"));
                keyStore = getKeyStore(userPKCS12, userPassword);
            } catch (Exception ex) {
                throw new RuntimeException("Could not decode userPKCS12", ex);
            }
        } catch (Exception ex) {
            throw new RuntimeException("Could not retrieve keystore from " + encodedUserPKCS12, ex);
        }
    ...
    public void createCredentials(TolvenPerson tolvenPerson, char[] userPassword) {
        KeyStore keyStore = createPKCS12KeyStore(tolvenPerson.getUid(), tolvenPerson.getCn(), tolvenPerson.getOrganizationUnitName(), tolvenPerson.getOrganizationName(), tolvenPerson.getStateOrProvince(), tolvenPerson.getUserPassword().toCharArray());
        byte[] userCertificate = getX509CertificateByteArray(keyStore);
        tolvenPerson.setUserCertificate(userCertificate);
