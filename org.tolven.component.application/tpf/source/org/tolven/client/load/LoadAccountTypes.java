    ...
    public void uploadApplications(String userId) throws Exception {
        try {
            if (!appFiles.keySet().isEmpty()) {
                ...
                ClientResponse response = webResource.type("multipart/mixed").cookie(getTokenCookie()).accept(MediaType.APPLICATION_FORM_URLENCODED).post(ClientResponse.class, multiPart);
