    ...
    public void execute(String[] args) throws Exception {
        ...
        for (Extension libExtension : abstractExtensionPoint.getConnectedExtensions()) {
            if (libExtension.getDeclaringPluginDescriptor().getId().equals(libPluginDescriptor.getId())) {
                for(Parameter parameter : libExtension.getParameters("lib")) {
                    String libFilename = parameter.valueAsString();
                ...
                for(Parameter parameter : libExtension.getParameters("dir")) {
                    String libDirname = parameter.valueAsString();
                    File sourceLibDir = getFilePath(libPluginDescriptor, libDirname);
                    logger.debug("Copy " + sourceLibDir.getPath() + " to " + destDir.getPath());
                    FileUtils.copyDirectoryToDirectory(sourceLibDir, destDir);
                }
            }
        }
    }
